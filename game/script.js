...
PHASES.length = Math.min(PHASES.length, 5);
const ALBUM_KEY = 'album_da_cozinha_ao_solo_v6';
const CONFIG = window.FIGURINHAS_CONFIG || { total: 36, arquivos: [] };

function getAlbumState(){
  try{ return JSON.parse(localStorage.getItem(ALBUM_KEY)) || {}; } catch(e){ return {}; }
}
function saveAlbumState(s){ localStorage.setItem(ALBUM_KEY, JSON.stringify(s)); }
function awardAlbumStickerByPhase(){
  const st = getAlbumState();
  st.inventario = st.inventario || {};
  st.coladas = st.coladas || {};
  st.gameFases = st.gameFases || {};
  if(st.gameFases[String(phaseIndex+1)]) return st.gameFases[String(phaseIndex+1)].figurinha || null;
  const available = [];
  for(let i=1;i<=36;i++){ if(!st.coladas[i]) available.push(i); }
  const base = available.length ? available : Array.from({length:36}, (_,i)=>i+1);
  const id = base[Math.floor(Math.random()*base.length)];
  st.inventario[id] = (st.inventario[id] || 0) + 1;
  st.gameFases[String(phaseIndex+1)] = { figurinha:id, data:new Date().toISOString() };
  saveAlbumState(st);
  return id;
}
...
const modalPremioImg = document.getElementById("modalPremioImg");
const modalPremioNumero = document.getElementById("modalPremioNumero");
const modalPremioArquivo = document.getElementById("modalPremioArquivo");
...
function play(audio, volume = 1) {
  ...
  try { audio.pause(); audio.currentTime = 0; audio.volume = Math.min(volume, 0.18); audio.play().catch(() => {}); } catch (e) {}
}
...
function notifyAlbumPhaseWon() {
  return awardAlbumStickerByPhase();
}
...
function checkWin() {
  ...
  if (score >= phase.pointsGoal && cactusBroken >= cactusTarget && getCoveredCount() === 0) {
    phaseFinished = true;
    const figurinhaGanha = notifyAlbumPhaseWon();
    btnProximaFase.disabled = phaseIndex >= PHASES.length - 1;
    markInteraction();
    play(audioVitoria, 0.9);
    const textoPremio = figurinhaGanha ? ` Você ganhou a figurinha ${String(figurinhaGanha).padStart(2,'0')} para colar no álbum.` : ' A recompensa desta fase já havia sido liberada anteriormente.';

    if (phaseIndex >= PHASES.length - 1) {
      openModal(
        "Missão concluída!",
        "Você concluiu todas as fases do game match." + textoPremio + " Volte ao álbum para colar sua recompensa.",
        { primaryText: "Reiniciar jogo", primaryAction: () => { closeModal(); resetGame(); }, secondaryText: "Voltar ao álbum", secondaryAction: () => { location.href='../index.html'; }, premioId: figurinhaGanha }
      );
    } else {
      openModal(
        "Fase vencida!",
        "Você concluiu a fase com sucesso." + textoPremio,
        { primaryText: "Próxima fase", primaryAction: () => { closeModal(); nextPhase(); }, secondaryText: "Voltar ao álbum", secondaryAction: () => { location.href='../index.html'; }, premioId: figurinhaGanha }
      );
    }
  }
}
...
function openModal(title, text, options = {}) {
  ...
  if (options.premioId) {
    const premio = String(options.premioId).padStart(2, "0");
    modalPremio.classList.remove("escondido");
    modalPremioImg.src = `../figurinhas/figurinha-${premio}.webp`;
    modalPremioNumero.textContent = `Figurinha ${premio} revelada`;
    modalPremioArquivo.textContent = `figurinha-${premio}.webp`;
  } else {
    modalPremio.classList.add("escondido");
    modalPremioImg.removeAttribute("src");
  }
